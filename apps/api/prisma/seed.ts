 ...
 * Run: npm run db:seed (from apps/api) or `npm run db:seed` from repo root.
 */
import bcrypt from "bcrypt";
import { PrismaClient } from "@prisma/client";
import { Decimal } from "decimal.js";
import crypto from "crypto";
import { bellCurve, jitter, eligibleCreditKwh } from "../src/lib/calculations";
...
async function main() {
  ...
  for (const { meter, zone, targetKwh } of meters) {
    ...
    for (const reading of mintableReadings) {
      ...
      const expiresAt = new Date(reading.timestamp.getTime() + 72 * 3_600_000);
      const credit = await prisma.energyCredit.create({
        data: {
          creditId: nextCreditId(zone.zoneCode, reading.timestamp),
      ...
      await prisma.meterReading.update({ where: { id: reading.id }, data: { creditIssued: true } });
      eligibleAccum.push(new Decimal(credit.availableKwh));
    }
